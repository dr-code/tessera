...
## Project Overview
Tessera is an open-source Python MCP server + CLI giving Codex persistent codebase memory via SQLite. No license gating.
...
- `src/tessera/debate/` — engine, codex wrapper, Codex wrapper, DLP sanitizer, XML payload
...
- Debate: GPT (Codex CLI subprocess) → Codex critique → GPT respond → Codex synthesize
...
### 9. AGENTS.md Is Team Memory
- When Codex makes a project-specific mistake, add the rule here
